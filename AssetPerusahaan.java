...
public class AssetPerusahaan {
    ...
    public void tampil() {
        if(list.isEmpty()) {
            ...
            System.out.printf("| %-8s | %-20s | %-20s | %-15s | %-15s", "ID ASSET", "NAMA ASSET", "DIVISI", "KUANTITAS", "HARGA");
            System.out.println("\n");
            for (Asset data : list) {
                
                System.out.printf("| %-8s | %-20s | %-20s | %-15s | %-15s", data.getIdAsset(), data.getNamaAsset(),
                        data.getDivisi(), data.getKuantitas(), data.getHarga() );
        }
        }
